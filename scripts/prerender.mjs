...
/* מפעיל האתר. תיקון 13 לחוק הגנת הפרטיות מחייב שהודעת הפרטיות תזהה את בעל
   המאגר, ולכן זה לא פרט קוסמטי אלא דרישה - והוא מופיע בשלושת העמודים
   המשפטיים מכאן, ממקום אחד, כדי שלא ייווצר הבדל ביניהם. */
const OWNER = 'טל יהודה';

/* דרך הפנייה היחידה באתר היא כפתור "הערה למנהל", ואין כתובת מייל מפורסמת.
   זו החלטה מודעת: כתובת שמתפרסמת בעמוד סטטי נאספת בידי בוטים לצמיתות.
   המשמעות היא שהטופס הזה הוא גם ערוץ מימוש הזכויות לפי החוק, ולכן הוא
   חייב להישאר זמין בכל מסך - הוא כבר שם, דרך NotesBox. */
const CONTACT = 'דרך כפתור <b>"הערה למנהל"</b> שנמצא בכל מסך באתר';
...
${crumbsHtml}`}
${body}
<footer>חלק מ<a href="/">ציר הזמן של עם ישראל</a> · מהאבות ועד חורבן בית שני · <a href="/p">מפת האתר</a> · <a href="/privacy">פרטיות</a> · <a href="/terms">תנאי שימוש</a> · <a href="/accessibility">נגישות</a></footer>
</div></body>
</html>`;
}
...
<p><b>מסירת הפרטים היא בחירה שלכם ואינה חובה.</b> אין חובה חוקית למסור שם, מייל או טלפון, ואפשר לכתוב תגובה או פנייה בלעדיהם. מי שלא משאיר דרך קשר פשוט לא יקבל תשובה חוזרת.</p>

<h2>כמה זמן נשמר המידע</h2>
<p><b>פניות פרטיות נמחקות שנה לאחר שטופלו</b>, על פרטי הקשר שבהן. אין סיבה להחזיק מייל וטלפון של מי שפנה פעם אחת לנצח, וככל שנשמר פחות - כך קטן הנזק אם משהו ידלוף.</p>
<p><b>תגובות פומביות נשמרות כל עוד הן מוצגות באתר</b>, כי הן חלק מהתוכן שגולשים אחרים קוראים. אפשר לבקש מחיקה בכל רגע, וזו זכות ולא בקשת טובה.</p>
<p>סטטיסטיקת השימוש נשמרת ללא הגבלת זמן - היא אינה מזהה אף אחד, ולכן אין ממה למחוק.</p>

<h2>הזכויות שלכם</h2>
<p>לפי חוק הגנת הפרטיות, ביחס למידע שנשמר עליכם עומדות לכם הזכויות האלה:</p>
<ul>
<li><b>לעיין</b> במידע שנשמר עליכם.</li>
<li><b>לתקן</b> מידע שאינו נכון, שלם או מעודכן.</li>
<li><b>לבקש מחיקה</b> של תגובה או של פנייה שכתבתם.</li>
</ul>
<p>למימוש כל אחת מהן פנו ${CONTACT}. ציינו מה כתבתם ומתי בערך, כדי שאוכל למצוא את זה - אין באתר חשבונות, ולכן אין דרך אחרת לזהות שהתגובה אכן שלכם.</p>

<h2>מי מחזיק את המידע</h2>
<p>האתר מסתייע בשלושה שירותים חיצוניים, וכל אחד מהם רואה רק את מה שנדרש לתפקידו:</p>
<ul>
<li><b>Supabase</b> - בסיס הנתונים שבו נשמרות התגובות והפניות.</li>
<li><b>Vercel</b> - אירוח האתר ומדידת צפיות בעמודים.</li>
<li><b>Resend</b> - שליחת המייל שמודיע לכם שהתקבלה תשובה לתגובה שלכם.</li>
</ul>
<p>שרתי השירותים האלה עשויים להימצא מחוץ לישראל, כמקובל בשירותי ענן.</p>
<p>האתר מיועד לקהל בישראל ואינו מכוון לגולשים באיחוד האירופי.</p>
...
<p>האתר מופעל בידי <b>${OWNER}</b>, כפרויקט אישי ללא מטרות רווח. בעל המאגר לעניין חוק הגנת הפרטיות הוא אותו אדם.</p>
<p>לכל פנייה בנושא פרטיות - עיון, תיקון, מחיקה או שאלה - פנו ${CONTACT}.</p>
<p>ראו גם: <a href="/terms">תנאי השימוש</a> ו<a href="/accessibility">הצהרת הנגישות</a>.</p>
<p class="dates">עודכן לאחרונה: ספטמבר 2026</p>`,
}));

/* תנאי שימוש. שני דברים מחייבים את העמוד הזה, ושניהם נובעים מכך שהאתר
   מציג תוכן שגולשים כתבו: ראשית, בלי הסכם אין רישיון לפרסם את התגובות
   שלהם. שנית, מפעיל אתר בישראל אינו אחראי לתגובה פוגענית שלא הכיר - אבל
   ברגע שקיבל פנייה ולא הסיר, האחריות עוברת אליו. הנוהל הכתוב כאן הוא
   ההגנה עצמה, ולכן הוא חייב להיות מנוסח כהתחייבות ולא כהצהרת כוונות. */
writeFileSync(join(DIST, 'terms.html'), shell({
  title: 'תנאי שימוש - ציר הזמן של עם ישראל',
  description: 'מה מותר לכתוב בתגובות, איך מבקשים הסרה, ומה מעמדו של התוכן באתר.',
  canonical: `${SITE}/terms`,
  crumbs: [{ name: 'ציר הזמן של עם ישראל', url: `${SITE}/` }, { name: 'תנאי שימוש' }],
  body: `<h1>תנאי שימוש</h1>
<p class="lead">בקצרה: התוכן כאן נועד ללמידה ולהתרשמות ולא לפסיקה או למחקר; מה שתכתבו בתגובות מוצג בפומבי; ותוכן פוגעני יוסר.</p>

<h2>מה האתר הזה, ומה הוא לא</h2>
<p>האתר מציג ציר זמן של תולדות עם ישראל מהאבות ועד חורבן בית שני. <b>כל השנים בו הן לבריאה לפי סדר עולם</b>, ולא לספירה.</p>
<p>התאריכים, המפות, המיקומים והזיהויים הם עיבוד של מקורות שונים, ובחלקם שנויים במחלוקת בין חוקרים ובין פרשנים. <b>אין להסתמך על האתר כמקור סמכותי</b> - לא לפסיקה הלכתית, לא לעבודה אקדמית ולא לכל הכרעה אחרת. מצאתם טעות? אשמח לתיקון, ${CONTACT}.</p>
<p>האתר ניתן לשימוש כמות שהוא. איני מתחייב שהוא יהיה זמין ברציפות, נקי משגיאות או מדויק בכל פרט.</p>

<h2>תגובות - מה מותר</h2>
<p>אפשר להגיב בלי הרשמה ובלי חשבון. <b>כל תגובה מוצגת בפומבי לכל מי שנכנס לאתר</b>, ולכן אין לכתוב בה פרטים אישיים שלכם או של אחרים.</p>
<p>אסור לפרסם באתר תוכן שהוא:</p>
<ul>
<li>פוגעני, מעליב, גזעני או מסית</li>
<li>לשון הרע על אדם או על גוף</li>
<li>פגיעה בפרטיותו של אדם אחר</li>
<li>הפרה של זכויות יוצרים - למשל העתקה של קטע מספר או ממאמר בלי רשות</li>
<li>פרסומת, קישורי ספאם או תוכן מסחרי</li>
<li>התחזות לאדם אחר</li>
</ul>

<h2>הסרת תוכן - איך ותוך כמה זמן</h2>
<p>נתקלתם בתגובה שמפרה את הכללים, פוגעת בכם או מפרה זכויות שלכם? <b>פנו ${CONTACT}</b>, וציינו באיזו תגובה מדובר ומה הבעיה.</p>
<p><b>אני מתחייב לבדוק כל פנייה כזאת בתוך 14 ימים</b>, ולהסיר תוכן שנמצא מפר. אם אחליט שלא להסיר, אשיב לפונה ואסביר מדוע.</p>
<p>זהו הגבול העליון ולא קצב העבודה: פנייה על תוכן פוגעני או משמיץ מטופלת מיד עם קבלתה, ולא ממתינה לסוף התקופה.</p>
<p>אני רשאי גם להסיר או לערוך תגובה ביוזמתי, בלי הודעה מראש, אם היא מפרה את הכללים שלמעלה.</p>

<h2>מה שאתם כותבים</h2>
<p>הזכויות בתגובה שכתבתם נשארות שלכם. בכתיבת תגובה אתם נותנים לי רשות להציג אותה באתר, לצטט ממנה בהקשר של האתר, ולתקן בה שגיאות כתיב וניסוח - ותו לא. <b>אתם רשאים לבקש את מחיקתה בכל עת</b>, ראו <a href="/privacy">מדיניות הפרטיות</a>.</p>
<p>האחריות לתוכן של תגובה היא של מי שכתב אותה.</p>

<h2>התוכן של האתר</h2>
<p>הטקסטים, ציר הזמן, המפות והעימוד הם יצירה מקורית. מותר לקרוא, לשתף קישורים, ולצטט בהיקף סביר עם ציון המקור. אין להעתיק את מאגר התוכן כולו או חלקים ניכרים ממנו לאתר אחר.</p>

<h2>שינויים בתנאים</h2>
<p>ייתכן שהתנאים האלה יעודכנו. הגרסה המחייבת היא זו המופיעה בעמוד זה, והתאריך שבתחתיתו מציין מתי עודכנה לאחרונה.</p>

<h2>דין וסמכות שיפוט</h2>
<p>על השימוש באתר חלים דיני מדינת ישראל, וסמכות השיפוט הבלעדית נתונה לבתי המשפט המוסמכים בישראל.</p>

<h2>מי מפעיל את האתר</h2>
<p>האתר מופעל בידי <b>${OWNER}</b>, כפרויקט אישי ללא מטרות רווח. לכל פנייה - ${CONTACT}.</p>
<p class="dates">עודכן לאחרונה: ספטמבר 2026</p>`,
}));

/* הצהרת נגישות. היא נדרשת ממי שמפעיל אתר, והיא הפער היחיד שבו האתר לא
   עמד בשום פרשנות - הקוד עצמו היה במצב טוב.

   מה שכתוב כאן חייב להישאר נכון: המספרים והכשלים שמפורטים בו נמדדו בפועל
   ב-axe-core, ומאותו יום השער ב-scripts/smoke.mjs מריץ את אותה בדיקה בכל
   דחיפה. כלומר אם ההצהרה תהפוך לשקר - השער יאדים לפני שגולש יגלה זאת.
   כל שינוי כאן מחייב הרצה מחדש של הבדיקה, לא ניסוח מחדש. */
writeFileSync(join(DIST, 'accessibility.html'), shell({
  title: 'הצהרת נגישות - ציר הזמן של עם ישראל',
  description: 'מה הונגש באתר, מה עדיין ייתכן, ואיך לדווח על תקלת נגישות.',
  canonical: `${SITE}/accessibility`,
  crumbs: [{ name: 'ציר הזמן של עם ישראל', url: `${SITE}/` }, { name: 'הצהרת נגישות' }],
  body: `<h1>הצהרת נגישות</h1>
<p class="lead">האתר נבדק מול תקן ת"י 5568 ברמה AA, והכשלים שנמצאו תוקנו. הבדיקה רצה מחדש בכל עדכון של האתר, כדי שהמצב הזה לא יישחק בשקט.</p>

<h2>מה נעשה</h2>
<p>האתר נבדק בכלי <span dir="ltr">axe-core</span> מול הנחיות <span dir="ltr">WCAG 2.1</span> ברמה AA - ההנחיות שתקן ישראלי ת"י 5568 מאמץ. הבדיקה רצה על כל אחד ממסכי האתר בדפדפן אמיתי.</p>
<p>בין ההתאמות שקיימות באתר:</p>
<ul>
<li>ניווט מלא במקלדת בין הפקדים, עם סימון ברור של הפקד הממוקד</li>
<li>יחסי ניגודיות של 4.5:1 לפחות בטקסט, כנדרש ברמה AA</li>
<li>תיאורים קוליים לקוראי מסך על כפתורים, מפות ואזורי הדף</li>
<li>מבנה כותרות ואזורים תקין, כדי שאפשר יהיה לדלג בין חלקי הדף</li>
<li>כיבוד הגדרת מערכת של "צמצום תנועה" - אנימציות נעצרות למי שביקש זאת</li>
<li>מקש <span dir="ltr">Esc</span> סוגר כל חלונית</li>
<li>האתר נפרש למסכי טלפון ומאפשר הגדלה עד 200% בלי אובדן תוכן</li>
</ul>

<h2>שלושה ליקויים שתוקנו</h2>
<p>בהצהרה הקודמת פורטו שלושה פערים ידועים. כולם תוקנו בספטמבר 2026:</p>
<ul>
<li><b>שדות הטפסים</b> בתגובות ובפנייה למנהל נשאו טקסט מרמז בלבד, שנעלם עם תחילת ההקלדה. לכל שדה יש כעת שם קבוע שקורא מסך מקריא.</li>
<li><b>רשימת תוצאות החיפוש ורשימת תחנות המסע</b> היו ניתנות לבחירה בעכבר בלבד. כעת אפשר להגיע אליהן במקלדת, והתחנה הנבחרת מדווחת לקורא מסך.</li>
<li><b>קישור "דלג לתוכן"</b> נוסף בראש שלושת המסכים.</li>
</ul>

<h2>מה עדיין ייתכן</h2>
<p>אני מעדיף לומר את זה במפורש מאשר להצהיר על התאמה מלאה. כלי בדיקה אוטומטי מכסה כשליש מדרישות התקן, ולכן ייתכנו ליקויים שלא נמצאו - ואם נתקלתם באחד, אשמח לדעת.</p>

<h2>דיווח על תקלת נגישות</h2>
<p>נתקלתם בקושי להשתמש באתר? <b>פנו ${CONTACT}</b>. תארו מה ניסיתם לעשות, באיזה מסך, ובאיזו טכנולוגיה מסייעת אם השתמשתם באחת.</p>
<p><b>אני מתחייב לבדוק כל פנייה בתוך שבעה ימי עסקים</b>, ולתקן ליקוי שנמצא בהקדם האפשרי.</p>

<h2>פרטי האתר והאחראי</h2>
<p>האתר מופעל בידי <b>${OWNER}</b>, כפרויקט אישי ללא מטרות רווח ואינו עסק. אותו אדם אחראי גם על הנגישות בו.</p>
<p>האתר בנוי כאתר עברי בכיוון ימין-לשמאל, ופועל בדפדפנים העדכניים במחשב, בטלפון ובטאבלט.</p>
<p class="dates">ההצהרה עודכנה בספטמבר 2026, בעקבות בדיקת נגישות ותיקון הליקויים שנמצאו בה.</p>`,
}));
