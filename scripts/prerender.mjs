...
writeFileSync(join(DIST, 'accessibility.html'), shell({
  ...
  description: 'מה הונגש באתר, מה עדיין לא, ואיך לדווח על תקלת נגישות.',
...
<h2>מה עדיין לא מונגש</h2>
<p>אני מעדיף לומר את זה במפורש מאשר להצהיר על התאמה מלאה שאינה מדויקת. שלושה פערים ידועים, וכולם בטיפול:</p>
<ul>
<li><b>שדות הטפסים</b> בתגובות ובפנייה למנהל מסומנים בטקסט מרמז בלבד, ולא בתווית קבועה. קורא מסך עשוי שלא להקריא את שם השדה לאחר תחילת ההקלדה.</li>
<li><b>רשימת תוצאות החיפוש ורשימת תחנות המסע</b> ניתנות לבחירה בעכבר או במגע, ולא במקלדת בלבד.</li>
<li><b>אין קישור "דלג לתוכן"</b> בראש הדף, ולכן משתמש מקלדת עובר את סרגל הניווט בכל כניסה לעמוד.</li>
</ul>
<p>בנוסף, כלי בדיקה אוטומטי מכסה כשליש מדרישות התקן. ייתכנו אפוא ליקויים שלא נמצאו - ואם נתקלתם באחד, אשמח לדעת.</p>
...
<p class="dates">ההצהרה עודכנה בספטמבר 2026, בעקבות בדיקת נגישות שבוצעה באותו חודש.</p>`,
}));
