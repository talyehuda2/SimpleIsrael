...
// "חברון (המכפלה)" ו-"שכם - קבורת עצמותיו" הם אותו מקום כמו חברון ושכם:
// שתי הצורות מתארות תחנה, ושתיהן הפיקו סמן נפרד במרחק אפס מהמקורי
const base = (name) => name.replace(/\s*\([^)]*\)\s*$/, '').split(/\s[-–]\s/)[0].trim();
